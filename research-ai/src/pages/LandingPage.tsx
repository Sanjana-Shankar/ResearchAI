...
const FEATURES = [
  {
    icon: '🔬',
    title: 'Research Mode',
    desc: 'Analyze papers, surface novel topics, and generate structured research proposals with timelines.',
  },
  {
    icon: '🚀',
    title: 'Product Dev Mode',
    desc: 'From idea to MVP — market analysis, competitor research, feature roadmaps, and tech stack recommendations.',
  },
  {
    icon: '🎯',
    title: 'Role-Tailored Output',
    desc: 'Outputs adapt to your role: PM, SWE, Marketing, or Researcher — so every insight is relevant.',
  },
]
    ...
    <div className="min-h-screen bg-slate-950 relative overflow-hidden">
      ...
      <nav className="relative z-10 flex items-center justify-between px-8 py-6 max-w-7xl mx-auto">
        ...
        {isAuthenticated && user ? (
          <button
            ...
            className="flex items-center gap-2.5 glass rounded-full px-3 py-1.5 hover:bg-white/10 transition-colors"
          >
            {user.picture ? (
      ...
      <main className="relative z-10 flex flex-col items-center text-center px-6 pt-20 pb-32 max-w-4xl mx-auto">
        ...
        <h1 className="text-5xl sm:text-6xl font-extrabold leading-tight tracking-tight mb-6">
          Turn ideas into{' '}
          <span className="text-gradient">structured plans</span>
          <br />
          in minutes
        </h1>
        ...
        <p className="text-slate-400 text-lg max-w-2xl mb-10 leading-relaxed">
          ...
          Choose your mode, define your goal, and get actionable roadmaps tailored to your role.
        </p>

        <button
          onClick={() => loginWithRedirect()}
          disabled={isLoading}
          className="group relative inline-flex items-center gap-2 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white font-semibold px-8 py-4 rounded-xl transition-all duration-200 glow-indigo hover:scale-105 active:scale-100"
        >
          {isLoading ? (
